...
# ------------------------------------------------
from _read_ import *
filename = os.path.join("..", "log", "pymmw_2022-01-13_15-31-38")
...
def onclick(event):
    ...
    if event.button == 3:  # toggle scale of datadict['dataFrame']
        global comp_choice
        comp_choice += 1
...
def update(datadict):
    ...
    if not 'range_doppler' in datadict['dataFrame'] or len(datadict['dataFrame']['range_doppler']) != range_bins * doppler_bins:
        return

    a = np.array(datadict['dataFrame']['range_doppler'])
